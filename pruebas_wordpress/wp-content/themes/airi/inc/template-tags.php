...
if ( ! function_exists( 'airi_posted_by' ) ) :
	...
	function airi_posted_by() {
		$byline = sprintf(
			...
			esc_html_x( 'De %s', 'post author', 'airi' ),
...
function airi_all_categories() {
	/* translators: used between list items, there is a space after the comma */
	$categories_list = get_the_category_list( esc_html__( ' ', 'airi_mod' ) );
	if ( $categories_list ) {
...
function airi_get_comments_number() {
	...
	if ( '1' === $comment_count ) {
		esc_html_e( '1 comentario', 'airi' );
	} else {
		printf( // WPCS: XSS OK.
			/* translators: 1: comment count number, 2: title. */
			esc_html( _nx( '%1$s comentario', '%1$s comentarios', $comment_count, 'Titulo', 'airi_mod' ) ),
